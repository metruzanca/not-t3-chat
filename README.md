...
- **Lightweight**
  - **Small memory footprint** - If self hosting as a webapp, the nodejs process shouldn't consume ~500mb+ ram just to idle. So we'll use rust instead. I want this to be able to run on my Raspberry Pi ZERO W (512mb ram).
  - **SQLite database** - We're selfhosting, not running a globally available realtime data (although sqlite/turso can and will do that). We want it simple and small.

Bonus:

- **Desktop application** as an option - For selfhosters who actually just want a local app. Dioxus supports this, so might as well try it.
